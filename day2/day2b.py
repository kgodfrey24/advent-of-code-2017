...
def splitAndConvertToFloat(stringInput):
    splitStrings = stringInput.split()
    return map(convertToFloat, splitStrings)

def convertToFloat(stringInput):
...
rows = input.split("\n")
rowsSplitByWhitespace = map(splitAndConvertToFloat, rows)

checksum = 0

for row in rowsSplitByWhitespace:
    sortedRow = sorted(row)

    divisorIndex = 0
    candidateIndex = 1
    foundValues = False
    while candidateIndex < len(sortedRow) and not foundValues:
        while candidateIndex < len(sortedRow) and not foundValues:
            divisorValue = sortedRow[divisorIndex]
            candidateValue = sortedRow[candidateIndex]

            if (candidateValue / divisorValue).is_integer():
                foundValues = True
                checksum = checksum + (candidateValue / divisorValue)
            candidateIndex = candidateIndex + 1

        divisorIndex = divisorIndex + 1
        candidateIndex = divisorIndex + 1
